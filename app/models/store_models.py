...
class store_goods_classify(db.Model):
    ...
    store_goods_classify_uuid = db.Column(db.String(128), unique=True, index=True, default='0')
    ...
    def __init__(self, **kwargs):
        super(store_goods_classify, self).__init__(**kwargs)

        if self.store_goods_classify_uuid is None:

            while True:
                self.store_goods_classify_uuid = str(uuid.uuid5(uuid.NAMESPACE_URL, str(uuid.uuid1())))
                checkuuid = store_goods_classify.query.filter_by(store_goods_classify_uuid=self.store_goods_classify_uuid).first()
